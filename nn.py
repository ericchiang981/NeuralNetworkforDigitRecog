...
class Graph(object):
    ...
    def add(self, node):
        ...
        # when node is type DataNode
        if isinstance(node, DataNode):
            self.outputs[node] = node.data
            self.inputs[node] = []
        # when node is type FunctionNode
        elif isinstance(node, FunctionNode):
            currentInputs = list()
            for parent in node.get_parents():
                currentInputs.append(self.outputs[parent])
            self.inputs[node] = currentInputs
            self.outputs[node] = node.forward(currentInputs)
    ...
    def backprop(self):
        ...
        for node in list(reversed(self.variables)):
            if(node == loss_node):
                self.gradients[node] = np.ones_like(self.outputs[node])
            currentGradient = self.gradients[node]
            currentInputs = self.inputs[node]
            outputGradient = node.backward(currentInputs, currentGradient)
            for parent, n in zip(node.get_parents(), range(len(node.get_parents()))):
                self.gradients[parent] = self.gradients[parent] + outputGradient[n]
    ...
    def step(self, step_size):
        ...
        for n in self.variables:
            if isinstance(n, Variable):
                n.data -= self.gradients[n] * step_size
...
class MatrixMultiply(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        product = np.dot(inputs[0], inputs[1])
        return product
    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        # print(gradient)
        # print(inputs[1].T)
        product1 = np.dot(gradient, inputs[1].T)
        product0 = np.dot(inputs[0].T, gradient)
        return [product1, product0]
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        gradientSum = np.sum(gradient, axis = 0)
        return [gradient, gradientSum]
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        array = np.subtract(inputs[0], inputs[1])
        array = np.multiply(array, array)
        array = np.multiply(array, 0.5)
        return np.sum(array) / array.size
    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        np.sum(inputs[0])
        gradient0 = gradient / inputs[0].size * np.subtract(inputs[0], inputs[1])
        gradient1 = gradient / inputs[1].size * np.subtract(inputs[1], inputs[0])
